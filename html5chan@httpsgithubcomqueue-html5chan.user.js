...
inject( "http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.js",
	...
	function parseThread(thread) {
		...
		return {
			...
			locked: thread.find('img[alt="closed"]').length > 0, //detect based on lock image
			preview: thread.filter('span.filesize').find('span[title]').length > 0, //previews on board page don't have a filename (detected with a span with the title attr)
			...
			sticky: (thread.find('img[alt="sticky"]').length > 0), //detect based on sticky image
	...
	Handlebars.registerPartial('post', '<header> <h1> <input type="checkbox" value="delete" name="{{id}}" form="delform"> <span class="title">{{title}}</span> <a class="poster" {{#if email}}href="mailto:{{email}}"{{/if}}>{{poster}}</a> <span class="tripcode">{{tripcode}}</span> <span class="capcode">{{capcode}}</span> <time pubdate datetime="{{time}}">{{datetime time}}</time> <a href="{{url}}" class="permalink" {{#if op}}target="_blank"{{/if}}>No.{{id}}</a> <button type="submit" form="reportform" name="no" value="{{id}}">[!]</button> </h1> {{#if image}}<div class="fileinfo"> <span class="dimensions">{{image.width}}x{{image.height}}</span> <span class="size">{{image.size}}</span> <span class="filename">{{image.filename}}</span> <a class="saucelink" href="http://iqdb.org/?url={{image.url}}" target="_blank">iqdb</a> <a class="saucelink" href="http://google.com/searchbyimage?image_url={{image.url}}" target="_blank">google</a> </div>{{/if}} </header> {{#if image}}  {{#with image.thumb}} <a class="file" target="_blank" href="{{../image.url}}"><img class="thumb" src="{{url}}" width="{{width}}" height="{{height}}"/></a> {{/with}}  {{/if}} <p class="comment"> {{{comment}}} </p>');
	Handlebars.registerPartial('thread','<article class="thread" id="{{op.id}}" tabindex="1"> <div class="op post"> {{#with op}} {{> post}} {{/with}} </div> {{#if omittedReplies}}{{#if replies}}<footer>{{numReplies}} replies and {{imageReplies}} image replies. Latest {{replies.length}} shown.</footer>{{/if}}{{/if}} <div class="replies"> {{#each replies}} <article class="post reply" id="{{id}}"> {{> post}} </article> {{/each}} </div> </article>');
